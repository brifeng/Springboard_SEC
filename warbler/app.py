...
def show_likes(user_id):
    ...
    likes = db.session.query(Likes).filter(Likes.user_id == user_id).all()

    likes_ids = []
    messages = []
    for like in likes:
        likes_ids.append(like.message_id)
        messages.append(
            db.session.query(Message).filter(
                like.message_id == Message.id).one())
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash(f"Saved profile changes, {user.username}!", "success")
...
def homepage():
    ...
    if g.user:
        ...
        likes = db.session.query(Likes).filter(
            g.user.id == Likes.user_id).all()
...
def remove_like(msg_id):
    ...
    if g.user:
        like = db.session.query(Likes).filter(
            g.user.id == Likes.user_id, msg_id == Likes.message_id).one()
